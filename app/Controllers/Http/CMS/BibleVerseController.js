...
class BibleVerseController {
  ...
  async index({ response }) {
    try {
      const bible_verses = await BibleVerses.query()
        .with("keywords")
        .with("mood_pivot.mood")
        .fetch();
  ...
  async search({ response, params: { id } }) {
    try {
      const rows = await BibleVerses.query()
        .where("id", id)
        .with("keywords")
        .fetch();
  ...
  async create({ request, response }) {
    try {
      ...
      const result = await this.bibleVersesServices.createBibleVerse({
        info: req,
      });
